...
const runBrowsers = async () => {
    ...
    for (const s of results) {
        ...
        const chooseName = () => {
            let name: string;
            if (s.name.split(" ")[2]) name = s.name.split(" ")[2];
            else if (s.name.split(" ")[1]) name = s.name.split(" ")[1];
            else if (s.name.split(" ")[0]) name = s.name.split(" ")[0];

            return name;
        }

        await page.goto(`${BASE_URL}/wp-admin/edit.php?s=${chooseName()}&post_status=all&post_type=staffmember&action=-1&m=0&paged=1&action2=-1`, {
            timeout: 0
        })
        ...
        for (const i in s.links) {
            console.log(i)
            await page.click("#acf-group_666d89734995b > div.inside.acf-fields.-top > div > div.acf-input > div > div.acf-field.acf-field-repeater.acf-field-666d8b4832a76 > div.acf-input > div > div > a")
        }
